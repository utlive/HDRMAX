...
if socket.gethostname().find('tacc')>0:
    ...
    out_root = '/scratch/06776/kmd1995/feats/feats/hdrdlm_nononliear/dlm_cr'
    out_pth = out_root
    ...
    out_root = '/media/zaixi/zaixi_nas/HDRproject/feats/hdrdlm_cb/dlm'
...
def dlm_refall_wrapper(ind,files):
    ...
    print(dis_filenames)
    Parallel(n_jobs=31,verbose=1)(delayed(dlm_video_wrapper)(ref_f,dis_f) for dis_f in dis_filenames)
...
def dlm_video_wrapper(ref_f,dis_f):
    ...
    dlm_image_wrapper(ref_f,dis_f,framenos,h,w)
    return
...
def dlm_image_wrapper(ref_f,dis_f,framenos,h,w,use_adaptive_csf=False,nonlinear = None,par = None,adaptation='bilateral',use_non_overlapping_blocks=False,use_views=False):
    ...
    with open(output_csv,'a') as f1:
        ...
        for framenum in range(framenos):
            try:
                _,ref_cb_pq,_ = hdr_yuv_read(ref_file_object,framenum,h,w)
                # print('The shape of cb:', ref_cb_pq.shape)
                _,dis_cb_pq,_ = hdr_yuv_read(dis_file_object,framenum,h,w)
            ...
            if(use_adaptive_csf==True):
                ...
                if(use_non_overlapping_blocks==True): # apply CSF on non-overlapping blocks of the image
                    csf_filtered_ref_cb_pq = blockwise_csf(ref_cb_pq)
                    csf_filtered_dis_cb_pq = blockwise_csf(dis_cb_pq)
                else: # sliding window; returns filtered value at center of each sliding window
                    csf_filtered_ref_cb_pq = windows_csf(ref_cb_pq,use_views=use_views,adaptation=adaptation)
                    csf_filtered_dis_cb_pq = windows_csf(dis_cb_pq,use_views=use_views,adaptation=adaptation)

                # standard DLM but without CSF
                dlm_val = dlm(csf_filtered_ref_cb_pq,csf_filtered_dis_cb_pq,use_csf=False)

            elif(nonlinear == 'logit'):
                logit_ref_cb_pq = logit(ref_cb_pq,1)
                logit_dis_cb_pq = logit(dis_cb_pq,1)
                dlm_val = dlm(logit_ref_cb_pq,logit_dis_cb_pq)
            elif(nonlinear == 'local_exp'):
                logit_ref_cb_pq = local_exp(ref_cb_pq,par,31)
                logit_dis_cb_pq = local_exp(dis_cb_pq,par,31)
                dlm_val1 = dlm(logit_ref_cb_pq,logit_dis_cb_pq)
                logit_ref_cb_pq = local_exp(ref_cb_pq,-par,31)
                logit_dis_cb_pq = local_exp(dis_cb_pq,-par,31)
                dlm_val2 = dlm(logit_ref_cb_pq,logit_dis_cb_pq)

            elif(nonlinear == 'global_exp'):
                logit_ref_cb_pq = global_exp(ref_cb_pq,par)
                logit_dis_cb_pq = global_exp(dis_cb_pq,par)
                dlm_val1 = dlm(logit_ref_cb_pq,logit_dis_cb_pq)
                logit_ref_cb_pq = global_exp(ref_cb_pq,-par)
                logit_dis_cb_pq = global_exp(dis_cb_pq,-par)
                dlm_val2 = dlm(logit_ref_cb_pq,logit_dis_cb_pq)
                ...
                # standard DLM 
                dlm_val = dlm(ref_cb_pq,dis_cb_pq,use_csf=True)
