...
if socket.gethostname().find('tacc')>0:
    ...
    out_root = '/media/zaixi/zaixi_nas/HDRproject/feats/hdrdlm_cr/dlm'
    ...
    with open(output_csv,'a') as f1:
        ...
        for framenum in range(framenos):
            try:
                ...
                # print('The shape of cr:', ref_cr_pq.shape)
            ...
            if(use_adaptive_csf==True):
                ...
                if(use_non_overlapping_blocks==True): # apply CSF on non-overlapping blocks of the image
                    csf_filtered_ref_cr_pq = blockwise_csf(ref_cr_pq)
                    csf_filtered_dis_cr_pq = blockwise_csf(dis_cr_pq)
                else: # sliding window; returns filtered value at center of each sliding window
                    csf_filtered_ref_cr_pq = windows_csf(ref_cr_pq,use_views=use_views,adaptation=adaptation)
                    csf_filtered_dis_cr_pq = windows_csf(dis_cr_pq,use_views=use_views,adaptation=adaptation)

                # standard DLM but without CSF
                dlm_val = dlm(csf_filtered_ref_cr_pq,csf_filtered_dis_cr_pq,use_csf=False)

            elif(nonlinear == 'logit'):
                logit_ref_cr_pq = logit(ref_cr_pq,1)
                logit_dis_cr_pq = logit(dis_cr_pq,1)
                dlm_val = dlm(logit_ref_cr_pq,logit_dis_cr_pq)
            elif(nonlinear == 'local_exp'):
                logit_ref_cr_pq = local_exp(ref_cr_pq,par,31)
                logit_dis_cr_pq = local_exp(dis_cr_pq,par,31)
                dlm_val1 = dlm(logit_ref_cr_pq,logit_dis_cr_pq)
                logit_ref_cr_pq = local_exp(ref_cr_pq,-par,31)
                logit_dis_cr_pq = local_exp(dis_cr_pq,-par,31)
                dlm_val2 = dlm(logit_ref_cr_pq,logit_dis_cr_pq)
                ...
                # standard DLM 
                dlm_val = dlm(ref_cr_pq,dis_cr_pq,use_csf=True)
