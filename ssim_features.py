...
def structural_similarity_features(im1, im2,
                                   *,
    ...
    pad = (win_size - 1) // 2
    prod = vx * vy
    prod[np.where(prod < 0)] = 0  # guard against divide by zero
    L = (2 * ux * uy + C1) / (ux ** 2 + uy ** 2 + C1)
    C = (2 * np.sqrt(prod) + C2) / (vx + vy + C2)
    # vxy[np.where(vxy < 0)] = 0  # guard against sqrt of negative
    S = (vxy + C2/2) / (np.sqrt(prod) + C2/2)
