...
if socket.gethostname().find('tacc')>0:
    ...
    out_root = '/scratch/06776/kmd1995/feats/feats/hdrvif_cb/vif'
    ...
    out_root = '/media/zaixi/zaixi_nas/HDRproject/feats/hdrvif_cb/vif'
        ...
        for framenum in randlist:
            ...
            if(use_adaptive_csf==True):
                ...
                if(use_non_overlapping_blocks==True): # apply CSF on non-overlapping blocks of the image
                    csf_filtered_ref_cb_pq = blockwise_csf(ref_cb_pq,adaptation=adaptation)
                    csf_filtered_dis_cb_pq = blockwise_csf(dis_cb_pq,adaptation=adaptation)
                else: # sliding window; returns filtered value at center of each sliding window
                    csf_filtered_ref_cb_pq = windows_csf(ref_cb_pq,use_views=use_views)
                    csf_filtered_dis_cb_pq = windows_csf(dis_cb_pq,use_views=use_views)

                # standard VIF but without CSF
                vif_val = vif(csf_filtered_ref_cb_pq,csf_filtered_dis_cb_pq)
            elif(nonlinear == 'logit'):
                logit_ref_cb_pq = logit(ref_cb_pq,1)
                logit_dis_cb_pq = logit(dis_cb_pq,1)
                vif_val = vif(logit_ref_cb_pq,logit_dis_cb_pq)
            elif(nonlinear == 'local_exp'):
                logit_ref_cb_pq = local_exp(ref_cb_pq,par,31)
                logit_dis_cb_pq = local_exp(dis_cb_pq,par,31)
                vif_val1 = vif(logit_ref_cb_pq,logit_dis_cb_pq)
                logit_ref_cb_pq = local_exp(ref_cb_pq,-par,31)
                logit_dis_cb_pq = local_exp(dis_cb_pq,-par,31)
                vif_val2 = vif(logit_ref_cb_pq,logit_dis_cb_pq)
                ...
                # standard VIF 
                vif_val = vif(ref_cb_pq,dis_cb_pq)
            # standard VIF
